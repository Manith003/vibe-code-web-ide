...
function Codes({ className, code, ...props }: CodeProps) {
  return (
    <CodeProvider value={{ code }}>
      <div
        className={cn(
          'relative flex flex-col overflow-hidden border rounded-lg bg-transparent',
          className,
        )}
        {...props}
      />
...
function CodeHeader({
  ...
  return (
    <div
      className={cn(
        'bg-neutral-800 shrink-0 gap-x-2 border-b border-border/75 dark:border-border/50 text-sm flex text-muted-foreground items-center px-4 w-full h-10',
        className,
      )}
      {...props}
    >
